...
//		login.loginButton.click(LoginPageDWS.class);
		...
		assertEquals(checkout1.BillingAddressDropDown.getSelectedOption(), data.get("Assertion3"),
				"Incorrect option selected from dropdown");

		checkout1.FirstName.enterText(data.get("FirstName"));
		assertEquals(checkout1.FirstName.getAttributeValue("value"), data.get("Assertion4"),
				"FirstName is not entered into FirstName textfield");

		checkout1.LastName.enterText(data.get("LastName"));
		assertEquals(checkout1.LastName.getAttributeValue("value"), data.get("Assertion5"),
				"LastName is not entered into LastName textfield");

		checkout1.Email.enterText(data.get("Email"));
		assertEquals(checkout1.Email.getAttributeValue("value"), data.get("Assertion6"),
				"Email is not entered into Email textfield");

		checkout1.CountryIdInBilling.selectByVisibleText(data.get("Country Dropdown Text"));
		assertEquals(checkout1.CountryIdInBilling.getSelectedOption(), data.get("Assertion7"),
				"Incorrect option selected from dropdown");

		checkout1.City.enterText(data.get("City"));
		assertEquals(checkout1.City.getAttributeValue("value"), data.get("Assertion8"), "Incorrect City");

		checkout1.Address1InBilling.enterText(data.get("Address1"));
		assertEquals(checkout1.Address1InBilling.getAttributeValue("value"), data.get("Assertion9"),
				"Incorrect Address");

		checkout1.ZipPostalCodeInBilling.enterText(data.get("Zip/Postal Code"));
		assertEquals(checkout1.ZipPostalCodeInBilling.getAttributeValue("value"), data.get("Assertion10"),
				"Incorrect Zip/Postal Code");

		checkout1.PhoneNumberInBilling.enterText(data.get("Phone Number"));
		assertEquals(checkout1.PhoneNumberInBilling.getAttributeValue("value"), data.get("Assertion11"),
