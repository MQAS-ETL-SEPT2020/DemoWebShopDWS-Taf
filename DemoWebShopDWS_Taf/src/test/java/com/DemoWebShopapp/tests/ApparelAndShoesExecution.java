...
public class ApparelAndShoesExecution extends BaseTest {
	...
	public void checkout(Map<String, String>data) throws InterruptedException {
		...
		assertEquals(login.email.getAttributeValue("value"),data.get("Username_Assertion"),"Email is not entered into the Email Textfeild");
		...
		assertEquals(login.password.getAttributeValue("value"),data.get("Password_Assertion"), "Password is not entered into the Password Textfield");
...
//		login.loginButton.click(LoginPageDWS.class);
        ...
        //assertTrue(checkout1.BillingAddressText.isVisible(), "checkout button is not clicked");
...
//	public void Checkout(Map<String, String>data) throws InterruptedException {
	...
	assertEquals(checkout1.FirstName.getAttributeValue("value"), data.get("Assertion2"), "First name is not entered into the firstname textfield");
	
	checkout1.LastName.enterText(data.get("LastName"));
	Log.info("LastName text field should be entered");
	assertEquals(checkout1.LastName.getAttributeValue("value"), data.get("Assertion3"), "Last name is not entered into the Lastname textfield");
	
	checkout1.Email.enterText(data.get("Email"));
	Log.info("Email text fiekd should be entered");
	assertEquals(checkout1.Email.getAttributeValue("value"), data.get("Assertion4"), "Email is not entered into the Email textfield");
	...
	assertEquals(checkout1.CountryIdInBilling.getSelectedOption(),data.get("Assertion5")," option is not selected");
	...
	assertEquals(checkout1.Address1InBilling.getAttributeValue("value"), data.get("Assertion7"), "Address is not entered");
	
	checkout1.ZipPostalCodeInBilling.enterText(data.get("Zip/Postal Code"));
	Log.info("Zip/Postal code should be enetered");
	assertEquals(checkout1.ZipPostalCodeInBilling.getAttributeValue("value"), data.get("Assertion8"), "Zip/postalcode is not entered");
	
	checkout1.PhoneNumberInBilling.enterText(data.get("Phone Number"));
	Log.info("Phone number should be entered");
	assertEquals(checkout1.PhoneNumberInBilling.getAttributeValue("value"), data.get("Assertion9"), "Phone number is not entered");
	
	checkout1.ContinueInBilling.click();
	Log.info("Clicked on continue button");
	//assertTrue(checkout1.ContinueInBilling.isVisible(), "continue button is not clicked");
	...
	Log.info("Clicked on continue button");
//	assertTrue(checkout1.isVisible(), "continue button is not clicked");
//	checkout1.ContinueinShipping.click();
// assertTrue(checkout1.CheckoutPage.isVisible(), "Checkoutpage is not
// displayed");
// checkout1.BillingAddress.selectByVisibleText(data.get("Dropdown Visible
// Text"));
