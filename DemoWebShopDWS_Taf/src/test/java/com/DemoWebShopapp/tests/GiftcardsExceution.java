...
public class GiftcardsExceution extends BaseTest {
	...
	public void Giftcards(Map<String, String> data) throws InterruptedException, AWTException {
		...
		// Log.info("Entering the data into password Textfeild");
		...
		assertEquals(giftspage.ReciepientsName_Textfeild.getAttributeValue("value"), data.get("Assertion3"),"Recipents Name is not entered in the Recipents Name Textfeild");
		...
		assertEquals(giftspage.ReciepentsMail_Textfeild.getAttributeValue("value"), data.get("Assertion4"),"Recipents Email is not entered in the Recipents Email Textfeild");
		...
		assertEquals(checkout.BillingAddressDropDown.getSelectedOption(), data.get("Dropdown Visible Text"),"Not Clicking on Billing Address option");

		checkout.FirstName.enterText(data.get("FirstName"));
		Log.info("Entering the data into the FirstName Textfeild");
		assertEquals(checkout.FirstName.getAttributeValue("value"), data.get("Assertion5"),"FirstName is not entered into the Firstname Textfeild");

		checkout.LastName.enterText(data.get("LastName"));
		Log.info("Entering the data into the LastName Textfeild");
		assertEquals(checkout.LastName.getAttributeValue("value"), data.get("Assertion6"),"LastName is not entered into the Lastname Textfeild");

		checkout.Email.enterText(data.get("Email"));
		Log.info("Entering the data into the Email Textfeild");
		assertEquals(checkout.Email.getAttributeValue("value"), data.get("Assertion7"),"Email is not entered into the Email Textfeild");
		...
		assertEquals(checkout.CountryIdInBilling.getSelectedOption(), data.get("Country Dropdown Text"),"Selected option is not selected");

		checkout.City.enterText(data.get("City"));
		Log.info("Entering the data into the City Textfeild");
		assertEquals(checkout.City.getAttributeValue("value"), data.get("Assertion8"),"City is not entered into the City Textfeild");

		checkout.Address1InBilling.enterText(data.get("Address1"));
		Log.info("Entering the data into the Address1 Textfeild");
		assertEquals(checkout.Address1InBilling.getAttributeValue("value"), data.get("Assertion9"),"Address1 is not entered into the Address1 Textfeild");
